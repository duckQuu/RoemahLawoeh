...
if (revealEls.length && 'IntersectionObserver' in window){
  const io = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      // Make the reveal animation re-play every time the element re-enters view.
      // When it leaves the viewport, remove the class so the CSS transition can run again.
      if (entry.isIntersecting) {
        // If it was previously revealed, force a reflow to ensure the transition re-triggers.
